...
def getModelLikelihood(exploit, belief_p_flip, trialStateActVals, subj_trials, beliefs_only=False, learn_p_flip=False, full_output=False):
    ...
    for trial in range(NUM_TRIALS):
        ...
        #        pre_vol = post_vol
        #        post_vol =   (num_jumps_seen+a0) / (num_jumps_seen+a0+(trial+1)-num_jumps_seen+b0) 
#          print trial, 'vol change', pre_vol, post_vol
#        if(abs(pre_vol - post_vol) > .005): 
#              old = preJumpBelief
#              preJumpBelief = recomputeBelief( model_trials, post_vol)
#              print '\tVOL CHANGE old:', old, 'new:', preJumpBelief

        # REWORK THIS PART TO DO GET OPTIMAL RESPONSE FOR EACH TRIAL
        if(full_output):
            softmax_p_exploit = getActionProbability(array([max(Q),min(Q)]), .16)
            softmax_trial_ll = log( [softmax_p_exploit,1.-softmax_p_exploit][choseExploit])
            Q[subj_trials[trial]['resp']] = subj_trials[trial]['reward']
            
            belief_p_exploit = getActionProbability(array([postJumpBelief, 1.-postJumpBelief]), .7)
            belief_trial_ll = log( [belief_p_exploit,1.-belief_p_exploit][choseExploit])

            model_trials.append( {'resp':subj_trials[trial]['resp'], 'reward':trial_reward, 'belief_before': postJumpBelief,
                                  'a_reward': subj_trials[trial]['a_reward'], 'b_reward': subj_trials[trial]['b_reward'],
                                  'opt_resp': {True:1, False:0}[trial_reward == max([subj_trials[trial]['a_reward'],
                                                                                     subj_trials[trial]['b_reward']])],
                                  'jumps_observed':jumpsObserved, 'explore':(1-choseExploit),
                                  'rel_val': exploitVal-exploreVal, 'actor_p_exploit':p_exploit,
                                  'opt_val':optActionValue, 'subopt_val':suboptActionValue,
                                  'actor_ll':log( [p_explore,p_exploit][choseExploit]),
                                  'rl_p_exploit':softmax_p_exploit,  'rl_ll': softmax_trial_ll,
                                  'belief_p_exploit':belief_p_exploit, 'belief_ll': belief_trial_ll   })
...
#temp = open('subjects_scr_dual_task.dat', 'rb')
...
for subj in [data[20],data[40]]:
    ...
    for exploit in linspace(.01, 1.0, 11):
        ...
        #belief_p_flip = 0.075 # added this to do fixed p_flip
            ll = getSoftmaxActorLikelihood([belief_p_flip], subj, exploit) #subj
            if (ll > best_subj_ll): [best_p_flip, best_exploit, best_subj_ll] = [belief_p_flip, exploit, ll]
    print subj[0][0], subj[0][1], best_subj_ll, getBIC(best_subj_ll, 2), best_p_flip, best_exploit


#for subj_num, subject in subjects.iteritems():
#    rect_trials = filter(lambda x:x['resp']!=-1, subject['trials'])
#    best_subj_ll = -inf
#    for exploit in linspace(.01, 1., 11):
#        for belief_p_flip in ALL_STATE_ACT_VALS.keys():
#            ll = getSoftmaxActorLikelihood([belief_p_flip], rect_trials, exploit)
#            if(ll > best_subj_ll): [best_p_flip, best_exploit, best_subj_ll] = [belief_p_flip, exploit, ll]
#    print subj_num, subject['cond'], best_subj_ll, best_p_flip, best_exploit




#for i, j in subjects.iteritems():
#    print i, j['cond'], j['actor_ll'], j['actor_p_flip']#, j['actor_exploit']
