...
def getModelLikelihood(exploit, belief_p_flip, trialStateActVals, subj_trials, beliefs_only=False, learn_p_flip=False, full_output=False):
    ...
    for trial in range(NUM_TRIALS):
        ...
        # subj cesd trial b_reward a_reward action choseExploit isBest explore_opt
        [p_exploit, p_explore] = [p_exploit, 1.-p_exploit]
        choseExploit = int(subj_trials[trial][6]) # CHANGED THIS
        ...
        if p_exploit >  p_explore:
            ...
            opt_res[trial] = 1
        # CHANGED THIS
        trial_reward = float(subj_trials[trial][ 3 + int(subj_trials[trial][5]) ])

        jumpsObserved = 0
        if(trial_reward > highestRewardSeen):
            jumpsObserved = (trial_reward - highestRewardSeen) / 10
            highestRewardSeen = trial_reward
            highestRawActionSeen = int(subj_trials[trial][5]) # CHANGED THIS
...
f = open(dir+'Leapfrog/fits/IA_fits_to_IA_sims/IA_fits_to_IA_sims_%s.txt' % (starting_sim+1), "w+")
...
for i in range(starting_sim, starting_sim + num_sims_to_fit):
    ...
    for subj in data[1:]:
        ...
        print subj[0][0], subj[0][1], best_subj_ll, getBIC(best_subj_ll, 2), best_p_flip, best_exploit, best_explore_opt, best_exploit_opt, best_explore_trials, best_exploit_trials, (i+1)
        f.write(str(subj[0][0])+' '+str(subj[0][1])+' '+str(best_subj_ll)+' '+str(getBIC(best_subj_ll, 2))+' ')
        f.write( str(best_p_flip)+' '+str(best_exploit)+' '+ str(best_explore_opt)+' '+str(best_exploit_opt)+' ') 
        f.write(str(best_explore_trials)+' '+str(best_exploit_trials)+' '+ str(i+1)+'\n')
        f.flush()
